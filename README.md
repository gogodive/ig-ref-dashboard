...
3. Claude 분석 — 새 게시물 한줄 기획 포인트 (캐시돼 있어 재분석 없음).
   ...
   리포트가 있으면 🔥 가 아니어도 버튼은 남는다
4. 🎯 심층분석 큐 갱신 — 3배↑ · **최근 1년 이내** · **게시 7일 경과**한 릴스만.
   **남의 계정에 올라간 공동 게시물은 배수를 안 내므로 🔥 도 큐도 안 붙는다** —
   조회수가 통째로 남의 오디언스라 우리 중앙값으로 나눌 근거가 없다
   (이 예외 없이 돌렸을 때 372.8배·239.4배가 나왔고 열어 보니 전부 가짜였다).
   우리 계정이 소유자면 상대를 몰라도 배수는 낸다.
   게시 직후 배수는 안 굳어서(D+3 에 3배가 D+30 에 1.5배가 되기도 한다) 익은 뒤에 넣는다.
   🔥 배지 자체는 기간을 안 보므로, 1년 넘은 🔥 에는 리포트 버튼이 안 붙는다
5. 대시보드 렌더 → GitHub Pages 배포.
   브랜드 안의 계정 탭은 **팔로워 많은 순** (팔로워 미수집 계정은 맨 뒤)
...
- Apify: 감지(계정수×10) + 창 밖 동결 전 URL 갱신 + 팔로워 주 1회.
  계정 37개 시점 실측 **일 ~615결과** (효율화 전 1,147결과 — 동결분 재구매 제거로 46% 절감).
  현재 43개이므로 계정수에 비례해 **일 ~700결과** 안팎으로 본다
